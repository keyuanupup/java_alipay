这里说一下大致的思路：查看支付宝的源码我们可以发现，支付宝的接口api上要求我们传递的是公钥证书所在的绝对路径，然后在源码中根据绝对路径去读取最后才拿到证书的数据流。所以我们就是传相对路径后修改他的读取操作。

具体代码如下：
```
LMAbstractAlipayClient
```
```
//
...
public abstract class LMAbstractAlipayClient implements AlipayClient {
    ...
    private <T extends AlipayResponse> void checkResponseCertSign(AlipayRequest<T> request, AlipayParser<T> parser, String responseBody, boolean responseIsSucess) throws AlipayApiException {
        ...
        } else if (certItem.getCert() == null && responseIsSucess && !request.getApiMethodName().equals("alipay.open.app.alipaycert.download")) {
